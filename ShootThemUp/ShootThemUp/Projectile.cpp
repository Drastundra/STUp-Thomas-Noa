...
Projectile::Projectile(const sf::Vector2f& position, float rotation)
    : mSpeed(800.0f) // Vitesse du projectile - à ajuster selon vos besoins
{
    mShape.setSize(sf::Vector2f(5.0f, 10.0f)); // Taille du projectile
    mShape.setFillColor(sf::Color::Yellow);
    mShape.setPosition(position);
    mShape.setRotation(rotation);

    // Calcul de la direction du projectile en fonction de la rotation
